...
> Auto-generated by Elyos task-decomposition on 2026-06-29. All 28 files live in `tasks/`.
> Seed file (001) was pre-existing; files 002–028 were generated in this pass.
> Fan-out: none (each backlog row maps to exactly one task file).

| File | Title | Type | Lane | Priority | Risk | Deliverable | Status |
|---|---|---|---|---|---|---|---|
| [ml-oncology-benchmarks-methodology-001.json](tasks/ml-oncology-benchmarks-methodology-001.json) | Leakage-aware benchmark methodology specification | design-spec | donated | high | medium | document | open |
| [ml-oncology-benchmarks-gate-002.json](tasks/ml-oncology-benchmarks-gate-002.json) | Licence + access-tier + de-identification gate checklist | design-spec | donated | high | medium | document | open |
| [ml-oncology-benchmarks-repo-003.json](tasks/ml-oncology-benchmarks-repo-003.json) | Monorepo skeleton: pnpm/TS/ESM harness + pinned Python ML container + CI | code | donated | high | low | pr | open |
| [ml-oncology-benchmarks-audit-004.json](tasks/ml-oncology-benchmarks-audit-004.json) | Leakage-audit suite skeleton (patient-overlap + preprocessing-isolation) | code | donated | high | medium | pr | open |
| [ml-oncology-benchmarks-datasheet-005.json](tasks/ml-oncology-benchmarks-datasheet-005.json) | Triage one candidate open dataset through the gate + publish its datasheet | data | donated | high | medium | dataset | open |
| [ml-oncology-benchmarks-outreach-006.json](tasks/ml-oncology-benchmarks-outreach-006.json) | Adopter/steward + expert-reviewer outreach | research | donated | high | low | document | open |
| [ml-oncology-benchmarks-subtype-007.json](tasks/ml-oncology-benchmarks-subtype-007.json) | Subtyping benchmark definition object + patient-grouped splits + external cohort | data | donated | high | medium | dataset | open |
| [ml-oncology-benchmarks-audit-008.json](tasks/ml-oncology-benchmarks-audit-008.json) | Full leakage-audit suite (feature/label-contamination, duplicate-sample, batch/centre-confound) | code | donated | high | medium | pr | open |
| [ml-oncology-benchmarks-baselines-009.json](tasks/ml-oncology-benchmarks-baselines-009.json) | Subtyping baselines (fit-on-train-only) + class-correct metrics + optimism-gap report | code | donated | high | medium | pr | open |
| [ml-oncology-benchmarks-modelcards-010.json](tasks/ml-oncology-benchmarks-modelcards-010.json) | Model cards + provenance ledger (code+data+split triple per number) | code | donated | high | medium | pr | open |
| [ml-oncology-benchmarks-repro-011.json](tasks/ml-oncology-benchmarks-repro-011.json) | One-command reproducibility re-run (pinned env, seeds) | code | donated | high | medium | pr | open |
| [ml-oncology-benchmarks-survival-012.json](tasks/ml-oncology-benchmarks-survival-012.json) | Survival benchmark definition (right-censored endpoint, nested CV, external cohort) | data | donated | medium | medium | dataset | open |
| [ml-oncology-benchmarks-survmetrics-013.json](tasks/ml-oncology-benchmarks-survmetrics-013.json) | Censoring-aware metric library (Harrell + Uno C-index, time-dependent AUC, integrated Brier score) | code | donated | medium | medium | pr | open |
| [ml-oncology-benchmarks-survaudit-014.json](tasks/ml-oncology-benchmarks-survaudit-014.json) | Temporal/lookahead leakage check added to audit suite | code | donated | medium | medium | pr | open |
| [ml-oncology-benchmarks-survbaselines-015.json](tasks/ml-oncology-benchmarks-survbaselines-015.json) | Survival baselines (Cox/penalized/RSF) fit-on-train-only + optimism-gap report | code | donated | medium | medium | pr | open |
| [ml-oncology-benchmarks-harness-016.json](tasks/ml-oncology-benchmarks-harness-016.json) | Refactor harness to run both task families from definition objects | code | donated | medium | medium | pr | open |
| [ml-oncology-benchmarks-submission-017.json](tasks/ml-oncology-benchmarks-submission-017.json) | Submission protocol + reproducibility re-run gate | code | donated | medium | medium | pr | open |
| [ml-oncology-benchmarks-leaderboard-018.json](tasks/ml-oncology-benchmarks-leaderboard-018.json) | Provenance-gated leaderboard (every row: external-cohort number + provenance) | code | donated | medium | medium | pr | open |
| [ml-oncology-benchmarks-breadth-019.json](tasks/ml-oncology-benchmarks-breadth-019.json) | Second cancer type/cohort benchmark via definition object | data | donated | medium | medium | dataset | open |
| [ml-oncology-benchmarks-adopt-020.json](tasks/ml-oncology-benchmarks-adopt-020.json) | Secure adopter/steward + record verified external reproductions and uses | research | donated | medium | medium | document | open |
| [ml-oncology-benchmarks-ops-021.json](tasks/ml-oncology-benchmarks-ops-021.json) | Ops/maintenance runbook + outcome dashboard + re-verification cadence | maintenance | donated | medium | medium | document | open |
| [ml-oncology-benchmarks-edu-022.json](tasks/ml-oncology-benchmarks-edu-022.json) | (Optional) Patient-facing explainer — education only (HIGH risk, blocked on oncologist+advocate) | writing | donated | low | high | document | open |
| [ml-oncology-benchmarks-proteomics-023.json](tasks/ml-oncology-benchmarks-proteomics-023.json) | Proteomic subtyping benchmark on open PRIDE/CPTAC data | data | donated | low | medium | dataset | open |
| [ml-oncology-benchmarks-multimodal-024.json](tasks/ml-oncology-benchmarks-multimodal-024.json) | Multi-omics (expression + clinical) survival benchmark | data | donated | low | medium | dataset | open |
| [ml-oncology-benchmarks-imaging-025.json](tasks/ml-oncology-benchmarks-imaging-025.json) | Open whole-slide pathology benchmark (open TCGA/CPTAC images only) | data | donated | low | medium | dataset | open |
| [ml-oncology-benchmarks-fairness-026.json](tasks/ml-oncology-benchmarks-fairness-026.json) | Subgroup/disparity evaluation across aggregate strata | code | donated | low | medium | pr | open |
| [ml-oncology-benchmarks-funded-027.json](tasks/ml-oncology-benchmarks-funded-027.json) | Funded-lane large-scale benchmark run (hard budget cap $50 USD) | code | funded | low | medium | pr | open |
| [ml-oncology-benchmarks-tutorial-028.json](tasks/ml-oncology-benchmarks-tutorial-028.json) | "How to evaluate oncology ML without leaking" teaching tutorial | writing | donated | low | low | document | open |
